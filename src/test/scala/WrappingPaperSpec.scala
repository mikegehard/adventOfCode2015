...
class WrappingPaperSpec extends FunSpec {
  describe("creating a present") {
    it("can be done from a string in the format l x w x h") {
  ...
  describe("Calculating wrapping paper for a single present") {
  ...
  describe("creating presents from a list of dimensions") {
    it("creates a present for each line") {
      val input = "2x3x4\n1x2x3"
      val expectedPresents = Array(XMasPresent("2x3x4"), XMasPresent("1x2x3"))

      assert(WrappingPaper.presentsFrom(input) === expectedPresents)
    }
  }
